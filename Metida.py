...
class P1:
    ...
    def move(self,lu,rb,gate,index,side,balls,your_team,enemy_team):
        ...
        if ((v1x*v2y - v2x*v1y)*((v1x*v3y - v3x*v1y)) < 0 and (balls[0][0] - your_position_x) * (0.5 - side) > 0) or (( your_position_x - balls[0][0])**2 + ( your_position_y - balls[0][1])**2   ) > 3500 :
            alpha = 10.0/max(abs(speed[0]),abs(speed[1]))
            speed = (alpha*speed[0], alpha*speed[1])
            return speed            
        
        if (( your_position_x - balls[0][0])**2 + ( your_position_y - balls[0][1])**2   ) > 400:
            target = nextCircleCoordinate( your_position_x,your_position_y, balls[0][0], balls[0][1], v1x*vty - v1y*v2x > 0 )
...
class P2:
    ...
    def move(self,lu,rb,gate,index,side,balls,your_team,enemy_team):
        ...
        if ((v1x*v2y - v2x*v1y)*((v1x*v3y - v3x*v1y)) < 0 and (balls[0][0] - your_position_x) * (0.5 - side) > 0 ) or (( your_position_x - balls[0][0])**2 + ( your_position_y - balls[0][1])**2   ) > 30000:
            alpha = 10.0/max(abs(speed[0]),abs(speed[1]))
            speed = (alpha*speed[0], alpha*speed[1])
            return speed            
...
class P3:
    ...
    def move(self,lu,rb,gate,index,side,balls,your_team,enemy_team):
        ...
        if (tx - your_position_x) ** 2 + (ty - your_position_y) ** 2 > 200  :
            target = nextCircleCoordinate( your_position_x,your_position_y, tx, ty, (tx-gx)*(your_position_y - gy) - (your_position_x - gx)*(ty-gy) > 0 )
            speed = (target[0] - your_position_x, target[1] - your_position_y) 
            alpha = 10.0/max(abs(speed[0]),abs(speed[1]))
            speed = (alpha*speed[0], alpha*speed[1])
            if speed[0] + your_position_x > rb[0] or speed[0] + your_position_x < lu[0]:
