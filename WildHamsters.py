...
def calcGateTarget(lu, rb, gate, side, enemy_team) :
    ...
    if getDist(gate_x, gate[0], enemy_team[3][0], enemy_team[3][1]) > getDist(gate_x, gate[1], enemy_team[3][0], enemy_team[3][1]) :
        gate_y = gate[0]
    else :
        gate_y = gate[1]
        ...
        if (getDist(my_x, my_y, balls[0][0], balls[0][1]) <= 50.0) :
            ...
            if (play_mode == 2 and ((balls[0][0] - (lu[0] + (rb[0] - lu[0]) * (0.3 + side * 0.4))) * (side - 0.5) > 0)) :
                ...
                if ((my_x - balls[0][0]) * (gate_x - balls[0][0]) + (my_y - balls[0][1]) * (gate_y - balls[0][1]) > 0) :
                    ...
                    if (getDist(my_x, my_y, balls[0][0] + 30.0 * v_1[0], balls[0][1] + 30.0 * v_1[1]) < getDist(my_x, my_y, balls[0][0] + 30.0 * v_2[0], balls[0][1] + 30.0 * v_2[1])) :
                        speed = getMaxVelocity(getNorm(balls[0][0] + 30.0 * v_1[0] - my_x, balls[0][1] + 30.0 * v_1[1] - my_y))
                    else:
                        speed = getMaxVelocity(getNorm(balls[0][0] + 30.0 * v_2[0] - my_x, balls[0][1] + 30.0 * v_2[1] - my_y))
                else:
    ...
    def move(self,lu,rb,gate,index,side,balls,your_team,enemy_team):
        ...
        if (getDist(my_x, my_y, balls[0][0], balls[0][1]) <= 20.0) :
            ...
            if ((balls[0][0] - (lu[0] + rb[0]) / 2) * (side - 0.5) > 0) :
                ...
                speed = getMaxVelocity(getNorm((lu[0] + (rb[0] - lu[0]) * (0.25 + side * 0.5)) - my_x, (balls[0][1] - my_y)))
        return speed
